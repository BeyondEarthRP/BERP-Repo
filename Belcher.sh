...
write_config() {
	...
	[[ -n "${REVIEW_CONFIGS:?}" ]] && pluck_fig "__REVIEW__" 10 false || __REVIEW__="false"
	if [ "$__REVIEW__" == "true" ] ;
	then
		unset __CONTINUE__
...
then
	[[ ! -f "${SERVER_CONFIG:?}" ]] \
	  && echo -e "\\e[91mYou must complete a belcher configuration before you can deploy. exiting.\\e[0m" \
	  && exit 1
	. "$BUILD/build-config.sh" DEPLOY
	. "$BUILD/build-config.sh" PERSONALIZE
	exit 0
fi
...
do
	...
	then
		...
		then
			...
			echo -e -n "\\x1B[97m\\x1B[44m\\x1B[K\\r"
			mysql -h db.local -u "${MYSQL_USER:?}" -p"${MYSQL_PASSWORD:?}" essentialmode < "${FILE:?}"
			if [ "$?" == "1" ] ;
			then
		...
		then # MAIN HEADER / TITLE
			...
			header_length="62"
			header_detail="${OUT_CFG:?}"
			header_sp2count=$(( "$header_length"-"${#header_detail}"-4 ))
                        [[ "${header_sp2count:?}" -lt 0 ]] && header_length=$(( ("${#header_detail}")+6 ))
                        [[ "${header_sp2count:?}" -lt 0 ]] && header_sp2count=$((header_length-4-("${#header_detail}") ))
			header_bar=$(printf '#%.0s' $(eval echo "{1..$header_length}"))
			header_spacer1=$(printf ' %.0s' $(eval echo "{1..$(($header_length-${#HEADER_INFO}-7 ))}"))
			header_spacer2=$(printf ' %.0s' $(eval echo "{1..$header_sp2count}"))

			echo -e -n "\\e[0m\\e[2K\\n\\e[0m\\e[2K\\t\\e[33m\\e[1m${header_bar:?}\\e[0m"
			echo -e -n "\\n\\e[0m\\e[2K\\t\\e[33m\\e[1m#\\e[100m  ${HEADER_INFO:?}) \\xe2\\x86\\x92${header_spacer1:?}\\e[0m\\e[33m\\e[1m#\\e[0m"
		...
		then #SUBHEADER
			...
			#$DATA
			subheader_length="62"
			subheader_detail="${OUT_CFG:?}"
			subheader_sp2count=$(( "$subheader_length"-"${#subheader_detail}"-6))
			[[ "${subheader_sp2count:?}" -lt 0 ]] && subheader_length=$(( ("${#subheader_detail}")+8 ))
			[[ "${subheader_sp2count:?}" -lt 0 ]] && subheader_sp2count=$((subheader_length-6-("${#subheader_detail}")))
			subheader_bar=$(printf '|%.0s' $(eval echo "{1..$subheader_length}"))
			subheader_spacer1=$(printf ' %.0s' $(eval echo "{1..$(( $subheader_length-${#DATA}-8 ))}"))
			subheader_spacer2=$(printf ' %.0s' $(eval echo "{1..$subheader_sp2count}"))

			echo -e -n "\\e[0m\\e[999D\\e[2K\\n\\e[999D\\e[2K\\t\\e[96m\\e[1m${subheader_bar:?}\\e[0m"
			echo -e -n "\\e[0m\\n\\e[999D\\e[2K\\t\\e[96m\\e[1m||\\e[0m\\e[30m\\e[46m  ${DATA:?} \\xe2\\x86\\x92${subheader_spacer1:?}\\e[0m\\e[96m\\e[1m||\\e[0m"
...
then
	...
	echo -e "\\e[93mAnswering 'No' here will require you to run: \\e[37m./Belcher.sh --Deploy\\e[0m"
