...
Citizen.CreateThread(function()
	while true do
		...
		if GUI.PhoneIsShowed then -- codes here: https://pastebin.com/guYd0ht4
			...
			if IsControlJustPressed(0, 0) and not IsInputDisabled(0) then
				keypressTimer = 0
				print("starting")
				while IsControlPressed(0, 0) do
					Citizen.Wait(5)
					keypressTimer = keypressTimer + 5
					print(keypressTimer)
					if keypressTimer > 100 then 
						break
					end
				end
				print(keypressTimer .. " moving on...")

				if not ESX.UI.Menu.IsOpen('phone', GetCurrentResourceName(), 'main') and keypressTimer > 200 then
					ESX.UI.Menu.CloseAll()
					ESX.UI.Menu.Open('phone', GetCurrentResourceName(), 'main')
				elseif ESX.UI.Menu.IsOpen('phone', GetCurrentResourceName(), 'main') then
					ESX.UI.Menu.CloseAll()
					ESX.UI.Menu.Close('phone', GetCurrentResourceName(), 'main')
				else
					keypressTimer = 0
			...
			if IsControlJustPressed(0, 45) and IsInputDisabled(1) then
				ESX.UI.Menu.CloseAll()
				ESX.UI.Menu.Close('phone', GetCurrentResourceName(), 'main')
			end 
		end
